...
> 건드리는 작업(`cmd/hippo/**`)은 시작 전 본 문서를 먼저 읽을 것.
...
2. **다중 데몬(Multi-Project) 아키텍처**:
   - 단일 데몬이 여러 프로젝트를 처리하는 대신, 프로젝트별로 완전히 격리된 별도의 데몬 프로세스를 구동합니다 (`hippo start <project>`).
   - 설정 파일, 포트, SQLite DB(Queue/Cache), PID 모두 `~/.hippodrop/projects/<project>/` 하위에 완벽히 격리됩니다.
   - `-c` 플래그나 `/etc/hippodrop/` 시스템 전역 설정 폴백은 배제하여 유저 권한 내에서 사이드 이펙트를 원천 차단합니다.
